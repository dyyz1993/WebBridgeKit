...
final class NotificationService: UNNotificationServiceExtension {
    ...
    private static func applyLongRingtone(
        ...
        let rawSound = (userInfo["aps"] as? [String: Any])?["sound"] as? String ?? ""
    ...
    private func recordPayload(_ userInfo: [AnyHashable: Any]) {
        ...
        for (rawKey, value) in userInfo where (rawKey as? String) != "aps" {
            guard let key = rawKey as? String, Self.isPlistValue(value)
            else { continue }
            dict[key] = value
        }
    ...
    /// iOS has no PropertyListSerialization.isValidPropertyList; whitelist
    /// plist-representable types so the write can never fail on exotic values.
    private static func isPlistValue(_ value: Any) -> Bool {
        switch value {
        case is String, is NSNumber, is Date, is Data:
            return true
        case let array as [Any]:
            return array.allSatisfy { isPlistValue($0) }
        case let dict as [String: Any]:
            return dict.values.allSatisfy { isPlistValue($0) }
        default:
            return false
        }
    }
