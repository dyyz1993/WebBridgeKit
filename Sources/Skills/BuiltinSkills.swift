...
/// All capabilities are registered with SkillRegistry.shared
public enum BuiltinSkills {

    // MARK: - Bridge

    public static let bridge = FrameworkCapability(
        name: "Bridge",
        category: "core",
        ...
        parameters: [
            Parameter(name: "name", type: "String", required: true, description: "Handler action 名称"),
            Parameter(name: "params", type: "Object", required: false, description: "Handler 参数 JSON 对象"),
            Parameter(name: "category", type: "String", required: false, description: "按分类过滤 Handler")
        ],
        tags: ["bridge", "js-native", "handler", "communication", "hardware"],
        examples: [
            "POST /tools/list_handlers?category=hardware",
            "POST /tools/get_handler_detail?name=camera.takePhoto",
            "POST /tools/execute_handler {\"name\":\"camera.takePhoto\",\"params\":{\"quality\":0.8}}"
        ]
    )

    // MARK: - Cache

    public static let cache = FrameworkCapability(
        name: "Cache",
        category: "core",
        ...
        parameters: [
            Parameter(name: "filter", type: "String", required: false, description: "按 key 前缀过滤"),
            Parameter(name: "prefix", type: "String", required: false, description: "按前缀清除缓存")
        ],
        tags: ["cache", "storage", "performance", "offline", "memory", "disk"],
        examples: [
            "POST /tools/get_cache_stats",
            "POST /tools/get_cache_entries?filter=images/",
            "POST /tools/clear_cache?prefix=temp/"
        ]
    )

    // MARK: - Message

    public static let message = FrameworkCapability(
        name: "Message",
        category: "core",
        ...
        parameters: [
            Parameter(name: "title", type: "String", required: true, description: "推送标题"),
            Parameter(name: "body", type: "String", required: true, description: "推送内容"),
            Parameter(name: "group", type: "String", required: false, description: "推送分组"),
            Parameter(name: "url", type: "String", required: false, description: "点击跳转 URL")
        ],
        tags: ["message", "push", "notification", "bark", "webhook"],
        examples: [
            "POST /tools/get_message_stats",
            "POST /tools/send_test_push {\"title\":\"Test\",\"body\":\"Hello\"}"
        ]
    )

    // MARK: - AI Debug

    public static let aiDebug = FrameworkCapability(
        name: "AI Debug",
        category: "debug",
        ...
        parameters: [
            Parameter(name: "name", type: "String", required: true, description: "工具名称"),
            Parameter(name: "method", type: "String", required: true, description: "MCP 方法名"),
            Parameter(name: "params", type: "Object", required: false, description: "方法参数对象")
        ],
        tags: ["debug", "http", "api", "mcp", "diagnostics"],
        examples: [
            "GET /health",
            "GET /tools",
            "POST /tools/get_diagnostic_report"
        ]
    )

    // MARK: - Theme

    public static let theme = FrameworkCapability(
        name: "Theme",
        category: "ui",
        ...
        apiEndpoints: [],
        parameters: [
            Parameter(name: "mode", type: "String", required: false, description: "主题模式：light/dark/system")
        ],
        tags: ["theme", "ui", "dark-mode", "light-mode", "design"],
        examples: [
            "ThemeManager.shared.apply(.dark)",
            "ThemeManager.shared.apply(.light)",
            "ThemeManager.shared.apply(.system)"
        ]
    )

    // MARK: - CommandParser

    public static let commandParser = FrameworkCapability(
        name: "CommandParser",
        category: "core",
        ...
        apiEndpoints: [],
        parameters: [
            Parameter(name: "command", type: "String", required: true, description: "命令字符串"),
            Parameter(name: "format", type: "String", required: false, description: "编码格式：json/url/base64")
        ],
        tags: ["parser", "command", "clipboard", "security", "decoder"],
        examples: [
            "CommandParser.shared.parse(<clipboard_string>)",
            "Configure CommandParser with maxPayloadSize: 4096"
        ]
    )

    // MARK: - Infrastructure

    public static let infrastructure = FrameworkCapability(
        name: "Infrastructure",
        ...
        parameters: [
            Parameter(name: "count", type: "Int", required: false, description: "最大条目数，默认 20"),
            Parameter(name: "level", type: "String", required: false, description: "最低日志级别：verbose/info/warning/error")
        ],
        tags: ["logging", "diagnostics", "infrastructure", "monitoring", "debug"],
        examples: [
            "POST /tools/get_recent_errors?count=50&level=warning",
            "POST /tools/get_diagnostic_report",
            "StructuredLogger.shared.getStats()"
        ]
    )

    // MARK: - All Skills

    public static let all: [FrameworkCapability] = [
        bridge,
        cache,
        message,
        aiDebug,
    ...
    /// Register all built-in skills with SkillRegistry
    /// - Throws: If any skill registration fails
    public static func registerAllWithRegistry() async throws {
        try await SkillRegistry.shared.registerAll(all)
    }
}
