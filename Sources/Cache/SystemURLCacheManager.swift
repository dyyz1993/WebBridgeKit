...
public class SystemURLCacheManager {
    ...
    /// 获取缓存统计信息
    /// - Returns: 统计信息
    public func getCacheStats() -> SystemCacheStatistics {
        statsLock.lock()
...
public class SystemCacheStatistics {
    ...
    public init(totalRequests: Int, cacheHits: Int, cacheMisses: Int, hitRate: Double, totalCacheSize: Int64) {
    ...
    public func updateAccessTime(_ time: TimeInterval) {
    ...
    private func updateHitRate() {
        let total = cacheHits + cacheMisses
        hitRate = total > 0 ? Double(cacheHits) / Double(total) : 0.0
    }
}
