...
本文档描述 WebBridgeKit SDK、AppTemplate 与完整 WebBridgeKit App 的共同产品需求、技术架构和实现规范。SDK 和模板服务于 App 开发，完整 App 是面向终端用户的产品交付物。

### 1.2 产品定位

WebBridgeKit 是一个以完整 iOS App 为最终交付目标的原生增强 PWA 项目：

1. `WebBridgeKit` SDK 提供可复用的 WebView、JS Bridge、离线缓存、消息路由、权限和诊断能力。
2. `AppTemplate` 提供使用 SDK 创建新宿主 App 的最小模板。
3. `SuperApp` 是基于 SDK 构建的完整 WebBridgeKit App，内部名称不等于“示例”；它承载普通用户使用的通知中心、PWA 应用中心、消息详情、审批与设置。
4. 官方托管版安装后直接使用；开源自托管版使用同一完整客户端，只额外进行一次网关导入。

SDK 的底层能力和调试能力是建设 App 的基础，不应反过来决定正式 App 的信息架构。正式 App 默认只呈现用户任务，框架调试功能集中到次级开发工具入口。

### 1.2.1 交付物边界

| 交付物 | 使用者 | 目标 | 不承担的职责 |
|---|---|---|---|
| WebBridgeKit SDK | iOS/PWA 开发者 | 提供跨宿主可复用能力和稳定协议 | 不定义某个产品的首页与业务流程 |
| AppTemplate | 需要开发自己宿主的团队 | 提供最小、可复制的 SDK 初始化起点 | 不作为官方 App，也不承载完整产品 UI |
| WebBridgeKit App (`SuperApp`) | 普通用户、官方版与自托管用户 | 提供完整通知和 PWA 宿主体验 | 不把 Debug/Showcase 当作主产品功能 |
| WebBridgeServer | 官方运营方或自托管部署者 | 提供推送、审批状态和兼容网关协议 | 不要求普通官方版用户部署基础设施 |
...
│                                                                  │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │              WebBridgeKit App (SuperApp)                │   │
│  │        (面向终端用户的完整产品，集成 SDK 能力)              │   │
│  └─────────────────────────────────────────────────────────┘   │
│                            │                                    │
│                            ▼                                    │
│  ┌─────────────────────────────────────────────────────────┐   │
