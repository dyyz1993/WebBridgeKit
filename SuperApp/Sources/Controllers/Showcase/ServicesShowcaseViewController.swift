...
class ServicesShowcaseViewController: UIViewController {

    private var tableView: UITableView!
    private var services: [(String, String)] = []

    private enum Section: Int, CaseIterable {
        case overview, services, actions
    }
    ...
    private func loadServices() {
        services = [
            ...
            ("Crash Service", "Crash reporting")
        ]
    }
}

extension ServicesShowcaseViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .overview:
            ...
            cell.configure(name: "Services Module", description: "Network services for API communication, authentication, and data sync.")
            return cell

        case .services:
            let cell = tableView.dequeueReusableCell(withIdentifier: "ServiceCell", for: indexPath) as! ServiceCell
            let (name, description) = services[indexPath.row]
        ...
        case .actions:
            ...
            let actions: [(String, ThemeButtonStyle, () -> Void)] = [
                ("Test API", .primary, { [weak self] in self?.testAPI() }),
                ("Check Auth", .secondary, { [weak self] in self?.checkAuth() })
            ]
...
private class ServiceCell: UITableViewCell {
...
extension ServicesShowcaseViewController {
    private func testAPI() {
        let alert = UIAlertController(title: "API Test", message: "Testing API service connectivity...", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "OK", style: .default))
        present(alert, animated: true)
    }

    private func checkAuth() {
        let alert = UIAlertController(title: "Auth Status", message: "Authentication token is valid.", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "OK", style: .default))
        present(alert, animated: true)
    }
}
